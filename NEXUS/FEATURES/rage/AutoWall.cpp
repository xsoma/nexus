...
{
	SDK::trace_t Trace;
	TraceLine(src, point, MASK_SHOT | CONTENTS_HITBOX, nullptr, &Trace);
...
float CAutowall::CanHit(Vector& vecEyePos, Vector& point)
{
	...
	TraceLine(data.src, end, MASK_SHOT | CONTENTS_HITBOX, local_player, &data.enter_trace);

	if (VectortoVectorVisible(data.src, vecEyePos))
	{
		return true;
	}
