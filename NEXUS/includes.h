...
#include "FEATURES/misc/EventLogs.h"
void RenderEventLog(std::string str, CColor clr, ...)
{
	if (!SETTINGS::settings.info_bool)
		return;

	INTERFACES::cvar->ConsoleColorPrintf(clr, prefix.c_str());
	INTERFACES::cvar->ConsoleColorPrintf(CColor(255, 255, 255), str.c_str());
	EventLog->messages.push_back(CMessage(str, gat::get_absolute_time(), clr));
}